...
7. Переходим по любой ссылке из контролеров, нас всё равно обяжут авторизоваться, авторизуемся под созданным юзером, далее нас перекинет на /api/user/home, мы должны получить json с нашим пользователем.
...
9. Можно создать ещё пару пользователей и проверить их всех по ссылке /api/user (пользователь должен иметь роль ADMIN). 
